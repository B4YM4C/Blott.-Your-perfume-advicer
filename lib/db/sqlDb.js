...
export const sqlDb = {
  ...
  async listCompletedSessions({ limit = 50, offset = 0 } = {}) {
    const rows = await q(
      `SELECT s.id                            AS id,
              ...
              EXTRACT(EPOCH FROM (s.completed_at - s.started_at)) * 1000 AS "durationMs",
   ...
   * One round-trip = one wide query per metric so it stays fast on Neon.
   */
  async dashboardSummary() {
    const num = (v) => Number(v ?? 0);
    // Postgres → INTERVAL '1 day', MySQL → INTERVAL 1 DAY
    const dayInterval = (n) => (_driver === 'mysql' ? `INTERVAL ${n} DAY` : `INTERVAL '${n} days'`);
    const [tot] = await q('SELECT COUNT(*) AS c FROM sessions WHERE completed_at IS NOT NULL');
    const [today] = await q(`SELECT COUNT(*) AS c FROM sessions WHERE completed_at >= NOW() - ${dayInterval(1)}`);
    const [week]  = await q(`SELECT COUNT(*) AS c FROM sessions WHERE completed_at >= NOW() - ${dayInterval(7)}`);
    const [special] = await q('SELECT COUNT(*) AS c FROM results WHERE special = TRUE');

    const [emailRow] = await q(
      `SELECT
         COUNT(*)                                      AS total,
    ...
    const [distRow] = await q('SELECT AVG(distance) AS avg_distance FROM results WHERE distance IS NOT NULL');
